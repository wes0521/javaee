    ...
    <div class="container-fluid">
      ...
      <div class="col-lg-12">
        <div style="float: left">
          <form class="form-inline" action="${pageContext.request.contextPath}/admin/member/findConsumeList">
            <div class="form-group">
              <label for="name">订单ID</label>
              <input type="text" class="form-control" id="name" name="consumeID" value="${consumeID}" >
            </div>
        ...
        <table class="table table-info table-striped shadow-info gradient-forest">
          ...
          <tbody>
          <c:forEach items="${consumeItems}" var="con" varStatus="s">
            <tr>
              <td scope="row">${s.count}</td>
              <td>${con.consume.consumeID}</td>
              <td>${con.user.userName}</td>
              <td>${con.cloth.clothName}</td>
              <td><f:formatDate value="${con.consume.consumeDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
              <td>${con.buyNum}</td>
              <td><span style="color: red">${con.consume.consumeSum}</span>&nbsp;元</td>
              <td>${con.consume.consumeState == 0 ? '未支付':'已支付'}</td>
              <td>
                <button type="button" onclick="removeConsumeA('${con.consume.consumeID}','${con.consume.consumeState}')" class="btn btn-danger btn-sm waves-effect waves-light m-1"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">删除</font></font></button>
                <%--<button type="button" onclick="removeConsumeA('${con.consume.consumeID}')" class="btn btn-danger btn-sm waves-effect waves-light m-1"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">删除</font></font></button>--%>
              </td>
            </tr>
          </c:forEach>
    ...
    layui.use('laypage', function(){
      ...
      laypage.render({
        ...
        ,jump: function(obj, first){
          ...
          if(!first){
            //改变切换页面的时候，再次查找数据
            window.location.href="${pageContext.request.contextPath}/admin/member/findConsumeList?pageSize="+obj.limit+"&pageIndex="+obj.curr+"&consumeID=${consumeID}";
          }
        }
      });
    });
